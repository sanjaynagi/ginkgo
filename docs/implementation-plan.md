...
### Phase 5 — Notebook and Script Task Contracts

**Goal:** Converge notebook execution onto the same explicit output contract used by other non-Python task kinds, add first-class script tasks for Python and R scripts that accept typed task inputs as CLI arguments, and smooth the authoring surface so task kind can be declared as the first positional argument to `@task(...)`.

**Downstream consumers:** Phase 12 (Publishing) bundles rendered notebook HTML and other declared report artifacts. The manifest format for notebook and script task outputs should remain explicit and stable.

#### Deliverables

- Replace the standalone `@notebook(...)` authoring surface with task-kind-based notebook execution:
  - Users declare notebook tasks as `@task("notebook")`.
  - `@task("notebook")` is the preferred spelling, while `@task(kind="notebook")` may remain supported for compatibility.
  - Task bodies return `notebook(...)`, analogous to `shell(...)`.
- Add a `notebook(...)` runtime primitive with explicit task-body configuration:
  - `nb=...` points to the source notebook.
  - `outputs=` is optional and may depend on resolved task inputs.
  - When `outputs` is provided, every declared output must be validated after execution.
  - When `outputs` is omitted, notebook-managed artifacts may still be produced and recorded in provenance.
- Preserve notebook-specific execution behavior under the hood:
  - `.ipynb` notebooks execute via Papermill with parameter injection.
  - marimo notebooks execute via CLI/script invocation with task inputs forwarded as CLI args.
  - HTML export and executed notebook artifacts remain notebook-specific provenance fields, but they are not part of the public output contract unless declared in `outputs`.
- Add a first-class `@task("script")` mode with a matching `script(...)` primitive:
  - Supports Python and R scripts.
  - Forwards resolved task inputs as CLI arguments.
  - Accepts optional `outputs=` with the same validation semantics as notebook tasks.
- Smooth the decorator surface across non-Python task kinds:
  - `@task("shell")`
  - `@task("notebook")`
  - `@task("script")`
  - `kind` should be accepted as the first positional argument to `@task(...)` rather than requiring `kind=...`.
- Keep task kind explicit rather than inferred from the returned primitive.
- Align return-value semantics across notebook and script tasks:
  - one declared output resolves to a single path-like value
  - multiple declared outputs resolve to a list/tuple matching the task annotation contract
  - omitted `outputs` resolves only the runtime-managed artifact contract, if any

#### Key design points

- `outputs` must live on `notebook(...)` and `script(...)`, not on a decorator, because output paths may depend on resolved task inputs.
- The authoring surface should optimize for readability over clever inference:
  - explicit `@task("notebook")` / `@task("script")` / `@task("shell")` is preferred
  - task kind should not be inferred from whether the body returns `notebook(...)`, `script(...)`, or `shell(...)`
- Notebook and script tasks should remain distinct from `shell(...)` even though they share output validation semantics:
  - notebook tasks still need parameter injection, execution/export steps, and notebook-specific provenance
  - script tasks still need interpreter selection and argument forwarding
- All declared outputs are part of the user-facing task contract and must be validated for existence after execution.
- Runtime-managed artifacts are additive provenance, not implicit public outputs, unless they are explicitly listed in `outputs`.
- Foreign environments should continue to support non-Python task kinds (`shell`, `notebook`, `script`) without requiring the Ginkgo runtime to be importable inside the target environment.

#### Validation

- Define a `@task("notebook")` task returning `notebook(path=..., outputs=[...])` and assert all declared outputs are validated and recorded in provenance.
- Define a notebook task without `outputs` and assert runtime-managed notebook artifacts are still produced and discoverable in the manifest.
- Define a `@task("script")` task for a Python script and assert resolved task inputs are forwarded as CLI args and declared outputs are validated.
- Define a `@task("script")` task for an R script and assert interpreter selection and output validation behave identically.
- Assert `@task("shell")`, `@task("notebook")`, and `@task("script")` are accepted as first-positional-kind spellings and remain equivalent to their keyword-based forms.
- Assert omitting the explicit task kind does not trigger automatic inference from the returned primitive; ambiguous or missing kinds should fail clearly.
- Assert single-output and multi-output notebook/script tasks resolve to values matching their task return annotations.
- Assert notebook and script tasks are cached by task inputs plus source content hashes for the underlying notebook or script file.
