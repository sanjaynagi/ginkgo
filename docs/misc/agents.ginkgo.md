...
- dynamic DAG expansion by returning `Expr`, `ExprList`, or `shell(...)` from tasks
...
Use `@task(kind="shell")` with `shell(...)` when the real work is a command-line tool.

Good:

```python
@task(env="tools", kind="shell")
...
- put the environment on `@task(env=...)`, not on `shell(...)`
...
- `shell(log=...)` is best for user-facing domain logs, not as the only place logs live
...
- using `shell(...)` without a meaningful `output`
...
> Build Ginkgo workflows using module-level `@task()` functions and one `@flow`. Keep task inputs explicit, use `file`/`folder`/`tmp_dir` annotations when path semantics matter, use `.map()` for fan-out, put dynamic branching inside task bodies, use `@task(kind="shell")` with `shell(...)` for CLI tools with explicit `output` paths, declare `env=` when reproducibility matters, and preserve cache correctness by bumping `version=` when task logic changes.
