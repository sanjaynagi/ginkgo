...
def annotate_compounds(
    ...
    -------
    pandas.DataFrame
        Annotated compound table. Wrapping with ``table(..., name=...)``
        registers it as an asset so the UI and `ginkgo asset show` render
        rich previews; downstream tasks still receive the plain DataFrame.
    """
